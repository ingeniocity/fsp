			...
			createConfTable += '<table class="table table-striped dTableR dataTable">';
				...
				for(var i=globalCountA; i<count; i++)
				{
					...
					createConfTable += '<tr>';
						...
						createConfTable += '<td align="left">'+grdData.data[i].FEEDER_SHUTDOWN_GROUP+'</td>';
...
function showPOTPlan(adata)
{
	...
	createConfTable += '<div class="row-fluid">';
		...
		createConfTable += '<tr>';
			...
			createConfTable += '<td>Total Relief</td>';
		createConfTable += '</tr>';
		for(i=0;i<(adata.data).length;i++)
		{
			createConfTable += '<tr>';
				...
				createConfTable += '<td>'+adata.data[i].GroupUsage+'</td>';
				createConfTable += '<td>'+adata.data[i].total+'</td>';
			createConfTable += '</tr>';
		}
	createConfTable += '</table>';
	createConfTable += '</div>';
...
function viewhistoryFm()
{
	...
	createConfTable += '<table width="100%">';
		...
		createConfTable += '<tr>';
			createConfTable += '<td><input class="UsrtextBox" type="text" name="batch" id="batch" value="" placeholder="Batch"></td>';
			createConfTable += '<td><input class="UsrtextBox" type="text" name="months" id="months" value="" placeholder="Months"></td>';
			createConfTable += '<td><input class="UsrtextBox" type="text" name="crops" id="crops" value="" placeholder="Crops"></td>';
			createConfTable += '<td><input class="UsrtextBox" type="text" name="capacity" id="capacity" onkeypress="return isNumber(event,0,this)" value="" placeholder="Capacity"></td>';
		createConfTable += '</tr>';
		createConfTable += '<hr>';
		createConfTable += '<tr>';
			createConfTable += '<td>&nbsp;</td>';
...
function viewgetFeederAddData(adata){
	...
	createConfTablef += '<option value="0">Select Feeder</option>';
	...
	createConfTablet += '<option value="0"> Select Talque</option>';
...
function add_feeder(){
		...
		var batch_sel = document.getElementById("section_sel").value;
		var month_sel = document.getElementById("revenue_sel").value;
		var crops_sel = document.getElementById("district_sel").value;
		var capicity_sel = parseInt(document.getElementById("talque_sel").value);
		if(!feeder)
		{
		...
		JSONObject.query = "call sp_11kv_createfeederhierarchy("+feed_sel+","+zone_sel+","+circle_sel+","+division_sel+","+sub_sel+","+station_sel+","+mp_sel+","+mla_sel+","+district_sel+","+talque_sel+","+gp_sel+","+section_sel+","+revenue_sel+",'"+batch_sel+"','"+crops_sel+"','"+month_sel+"','"+capicity_sel+"','Y','','"+feeder+"','"+feeder_scada+"',"+compid+")";
		JSONstring = JSON.stringify(JSONObject);
		//document.getElementById("test_AddFM").innerHTML ="call sp_11kv_createfeederhierarchy("+feed_sel+","+zone_sel+","+circle_sel+","+division_sel+","+sub_sel+","+station_sel+","+mp_sel+","+mla_sel+","+district_sel+","+talque_sel+","+gp_sel+","+section_sel+","+revenue_sel+",'"+batch_sel+"','"+crops_sel+"','"+month_sel+"','"+capicity_sel+"','Y','','"+feeder+"','"+feeder_scada+"',"+compid+")";
		//document.getElementById(div_id).style.display="none";
		runAjax(parser_page,JSONstring);
...
function viewagetTalque(adata){
	...
	createConfTable += '<option value="0">Select Talque</option>';
	for(i = 0; i < len; i++)
		createConfTable += '<option value="'+adata.data.SPS[i].GeoID+'">'+adata.data.SPS[i].GeoName+'</option>';
	createConfTable += '</select>';
	createConfTablegp += '<select id="gp_sel">';
