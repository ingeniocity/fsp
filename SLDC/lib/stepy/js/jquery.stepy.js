...
;(function($) {
	...
	var methods = {
		init: function(options) {
        	...
        	$('<a/>', { id: id + '-next-' + index, href: 'javascript:void(0);', 'class': 'btn btn-inverse button-next', html: opt.nextLabel }).click(function() {
				//alert(index+"___"+allowedFlag+"___"+contFlag);
				if(allowedFlag && contFlag == 0)
				{
					if(index == 0 && allowedFlag < 1)
						wizard_opr_next(index+1);
	
					if(index == 1 && allowedFlag < 3)
						wizard_opr_next(index+1);
						
					if(index == 2 && allowedFlag < 4)
						wizard_opr_next(index+1);
				}
				else if(contFlag == 0)
					wizard_opr_next(index+1);
				else if(index == 0 && allowedFlag == 0 && contFlag == 1)
					wizard_opr_next(index+1);
					
				if(allowedFlag >= 1 && contFlag && index == 0)
				{
